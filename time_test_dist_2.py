...
def main():
    ...
    timefile = open('exectime_DIST_2.txt','w') #ouverture du fichier qui va contenir tous les données de temps resolution. #format fichier -> @taille_x @taille_y @temps @nom_du_fichier
    i = 0
    start_time = 0
    end_time = 0
    ...
    while(i<len(fichiers) and (end_time-start_time)<600):
        ...
        if(os.path.isfile(os.path.join(folderpath,fichiers[i])) and os.path.splitext(fichiers[i])[1] == ".adn"):
            ...
            dist = DIST_2.dist_2(struct_sequences["X"],struct_sequences["Y"])
            end_time = time.time()
